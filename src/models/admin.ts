...
const Admin = sequelize.define('Admin', {
    userId: {
        ...
        primaryKey: true,
        autoIncrement: true, // 确保添加了这一行
    },
    username: DataTypes.STRING,
    email: DataTypes.STRING,
    password: DataTypes.STRING,
    canAdmin: DataTypes.BOOLEAN,
    phoneNumber: {
...
async function findUserById(userId: number): Promise<UserInfo | null> {
    try {
        ...
        const userInstance = await Admin.findOne({
...
async function updateUser(userId: number, userData: Partial<UserInfo>) {
    try {
        ...
        const [affectedCount] = await Admin.update(userData, {
            where: { userId: id },
        }).catch(error => {
...
// 新增用户
async function createUser(newUser: Omit<UserInfo, 'userId'>): Promise<UserInfo | undefined> {
    try {
